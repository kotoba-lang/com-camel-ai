(ns camel-ai.main
  "Kotodama WASM entrypoint for the camel_ai clean-room actor (L4) — Clojure port.
  ...
  facts (`camel_ai.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "camel_ai")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"camel_ai.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "camel_ai-compat" :tier tier :entities entities} 200])
...
(defn start! [] :camel_ai-compat/ready)
